  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-6">
        <Link href={`/company/invoices/${id}`} className="text-gray-400 hover:text-gray-600">
          <ArrowLeft size={20} />
        </Link>
      ...
      <div className="card text-center py-8">
        ...
        <p className="text-sm text-gray-500 mb-6">
          {invoice.users?.name} 様宛
        </p>

        <PDFDownloadLink
          document={<InvoicePDF {...pdfProps} />}
          fileName={`invoice_${invoice.year}${String(invoice.month).padStart(2, '0')}_${invoice.users?.name}.pdf`}
        >
          {({ loading: pdfLoading }) => (
